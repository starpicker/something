...
unsigned char*
bmp_get_rgb32(bmpfile_t *bmp, unsigned char *rgb32)
{
  int row;
  unsigned char *buf;
  ...
  if (bmp->dib.depth == 16) {
    ...
    unsigned char * buf1 = (unsigned char*)malloc(bytes_per_line);

    // for (row = bmp->dib.height - 1; row >= 0; --row) {
    for (row = 0; row <= bmp->dib.height - 1; ++row) {
      memset(buf1, 0, bytes_per_line);

      switch (bmp->dib.depth) {
      case 1:
	bmp_get_row_data_for_1(bmp, buf1, bytes_per_line, row);
	break;

      case 4:
	bmp_get_row_data_for_4(bmp, buf1, bytes_per_line, row);
	break;

      case 8:
	bmp_get_row_data_for_8(bmp, buf1, bytes_per_line, row);
	break;

      case 24:
	bmp_get_row_data_for_24(bmp, buf1, bytes_per_line, row);
	break;

      case 32:
	bmp_get_row_data_for_32(bmp, buf1, bytes_per_line, row);
	break;
      }

        memcpy(buf, buf1, bytes_per_line);
        buf += bytes_per_line;
    }
    free(buf1);
  }
