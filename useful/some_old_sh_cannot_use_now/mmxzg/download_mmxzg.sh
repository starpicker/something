...
grep -v "index" $SITEMAP | grep -o $LIST_PATTEN > $IMAGES_DAYLY_LIST
...
cat $ADD_LIST | xargs curl | grep "img alt" | grep -o $IMAGES_URL_PATTEN | cut -d"\"" -f2 | xargs wget -t10 -T15
