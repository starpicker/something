...
$DIFF $IMAGES_DAYLY_LIST $TOTAL_LIST | grep "-" | $AWK -F'-' '{print $2}' > $ADD_LIST 

cat $ADD_LIST >> $TOTAL_LIST

cat $ADD_LIST | $XARGS curl | grep $IMAGES_PAGE_URL_PATTEN | grep -o $IMAGES_URL_PATTEN > jpeg_urls.txt
#| $XARGS $WGET -nc -t10 -T15
