...
int read_JPEG_file(string strImageName)
{
    ...
    if(cinfo.out_color_space == JCS_YCbCr)
     ...
     stringstream ss;
     ss << cinfo.output_width << "x" << cinfo.output_height << ext;
     
     ofstream os;
     os.open(ss.str().c_str(), ios::binary);
     ...
     */
        ...
        os.write((const char*)*buffer, row_stride);
...
int Bmp2Jpeg_Compress(void* lpBmpBuffer, int nWidth, int nHeight,  void** ppJpegBuffer,  unsigned long* pOutSize)
{
	...
	//注册失败的回调函数
	toWriteInfo.err->error_exit = jerror_exit;
	...
	toWriteInfo.in_color_space = JCS_EXT_RGBA;//JCS_EXT_BGRA; //JCS_GRAYSCALE表示灰度图，JCS_RGB表示彩色图像 
...
int main(int argc, char* argv[])
{
    if(argc != 5)
    {
        printf("%s input.rgb32 output.jpg width height\n", argv[0]);
        return -1;
    }
...
Bmp2Jpeg_Compress(data32, width, height,  &pJpegBuffer,  &lOutSize);

   FILE* fpOut = fopen(argv[2], "w");  
        if (fp)  
        {  
            fwrite(pJpegBuffer, 1, lOutSize, fpOut);  
            fclose(fpOut);  
        }  
